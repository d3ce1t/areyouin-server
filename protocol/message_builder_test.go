...
func TestOkMessage3(t *testing.T) {
	// Check header
	msg := &Ok{Type: uint32(M_USER_AUTH)}

	if msg.Type != uint32(M_USER_AUTH) {
		t.Fail()
	}
}
...
func TestOkMessage6(t *testing.T) {
	// Check header
	msg := NewPacket(VERSION_2).Ok(234)
...
func TestErrorMessage1(t *testing.T) {
	msg := &Error{Type: uint32(M_USER_CREATE_ACCOUNT), Error: E_EMAIL_EXISTS}
	if msg.Type != uint32(M_USER_CREATE_ACCOUNT) || msg.Error != E_EMAIL_EXISTS {
		t.Fail()
	}
}
